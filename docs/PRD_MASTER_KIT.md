# PRD: Master-Kit Orchestrator Monorepo
...
The master hook at `.claude/hooks/pre-tool-use.sh` must:
...
- avoid recursive re-entry when dispatching between master/kit hooks
...
- Auth: `Authorization: Bearer <MASTER_KIT_MCP_TOKEN>`
...
All tool outputs must remain pointer-oriented and bounded by `MASTER_KIT_MCP_MAX_OUTPUT_BYTES`.
